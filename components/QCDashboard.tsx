...
const QCDashboard: React.FC<QCDashboardProps> = ({ machines }) => {
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white">品質管理 (QC 抽檢)</h2>
          ...
          <div key={machine.id} className="bg-slate-800 rounded-xl border border-slate-700 p-6">
             <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-700">
               <div>
                 ...
                 <p className="text-sm text-slate-400">當前批號: <span className="text-brand-400 font-mono">{machine.currentLot}</span></p>
               </div>
               <div className="text-right">
                 <span className="text-sm text-slate-400 block">批次良率</span>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                   <h4 className="text-sm font-semibold text-slate-300 mb-4">缺陷分佈</h4>
                   <div className="h-64">
                ...
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold text-slate-300">缺陷計數詳情</h4>
                  ...
                  <div className="mt-6 p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
                    <p className="text-xs text-red-400 font-semibold uppercase">需要採取行動</p>
                    <p className="text-sm text-slate-300 mt-1">請檢查批號 {machine.currentLot} 的尺寸，發現微量偏差。</p>
                  </div>
                </div>
             </div>
          </div>
