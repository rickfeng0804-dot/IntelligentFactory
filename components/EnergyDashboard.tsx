  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-white">能源管理看板 (Energy Management)</h2>
        <div className="bg-slate-800 px-4 py-2 rounded-lg border border-slate-700 flex items-center gap-3">
          <Zap className="text-yellow-400" size={20} />
          <div>
            <p className="text-xs text-slate-400">單日總耗電量</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {blocks.map((block) => (
          <div key={block.id} className="bg-slate-800 rounded-xl border border-slate-700 p-4">
            ...
            <p className="text-xs text-center text-slate-500 mt-2">24小時趨勢</p>
          </div>
        ))}
      </div>
    </div>
